...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{lcccc}
			\hline
			\textbf{Fold no.} & \textbf{Epoch (LR Reduced)} & \textbf{Learning Rate After Reduction} & \textbf{Early Stopping Epoch} & \textbf{Best Epoch (Restored)} \\ \hline
...
Figure~\ref{fig:roc_auc} shows the ROC Curve shows the ability of the proposed model to correctly identify the true positives, which can help determine the tradeoff between accuracy and sensitivity.  It will also determine the validity of the model, that it is not predicting based only on random chances. The range of AUC ROC is between 0.5 and 1. The model was able to achieve a score of 0.7734, which is better than random chances and an indication that the model is performing reasonably. 
